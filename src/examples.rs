...
pub fn load_stdlib(repl: &mut Repl) {
    ...
    repl.read_line("nil = false");
    repl.read_line("isnil = \\xs. xs (\\h.\\t. false) true");
    ...
    // TODO: fix recursive defs
    repl.read_line(
        "map = fix (\\m.\\f.\\xs. if (isnil xs) nil (cons (f (fst xs)) (m f (snd xs))))",
    );
}
...
pub fn load_qntmlib(repl: &mut Repl) {
    ...
    repl.read_line("deutsch = \\U.(\\p.cons (H (fst p)) (snd p)) (U (cons (H |0>) (H |1>)))")
}
